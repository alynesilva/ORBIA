...
I, as the user, want to create an account on the site and to do pucharce
  ...
  Scenario: Create a new account on the site from the store with success
    ...
    When the user insert the e-mail "marisasilva@orbia.ag" and  click on button "Create an account"  
    ...
    Then, next, the user  insert the fields text for First Name (John ) and Last Name (Lennon)  and Password (mmKB98jU)
    ...
    Then the user in field "YOUR ADDRESS	", the user must insert: the First and Last  name, the First and second Address(Line 2), which can be Apartment, suite, unit, building, floor, etc...        
    And too must insert, the City, the State, the postal code and select an Country and enter additional information 
